...
def calculateVendorSalesMetricsByCustomer(vendor)
  BiCustomer.all.each do |bi_cust|
   ...
   SalesMetric.create(metric_type: SalesMetric.metric_types[:vendor_costs],value_cents:total_cost_cents,measured_count:order_line_items_count, measured_id:vendor.no, bi_customer_no: bi_cust.no )
